...
FEEDS = [
    ...
    # X/Twitter bronnen via Nitter (open-source Twitter-mirror met RSS)
    # Als één Nitter-instance offline gaat, kun je de host swappen voor een andere:
    # zie https://github.com/zedeus/nitter/wiki/Instances voor actuele lijst
    {
        "naam": "X: @OnsOranje",
        "url": "https://nitter.privacydev.net/OnsOranje/rss",
    },
    {
        "naam": "X: @KNVB",
        "url": "https://nitter.privacydev.net/KNVB/rss",
    },
    {
        "naam": "X: @FIFAWorldCup",
        "url": "https://nitter.privacydev.net/FIFAWorldCup/rss",
    },
]
...
def classificeer(titel: str, samenvatting: str, bron: str = "") -> dict:
    """Bepaal categorie en of het 'hot' is."""
    volledige_tekst = f"{titel} {samenvatting}"

    # X-accounts zijn al voorgefilterd op relevantie (OnsOranje = altijd Oranje, etc)
    is_x_bron = bron.startswith("X:")
    if is_x_bron:
        if "OnsOranje" in bron or "KNVB" in bron:
            categorie = "oranje"
        else:
            categorie = "algemeen"
        # Trefwoorden voor 'hot' wel checken
        is_hot = bevat_trefwoord(volledige_tekst, HOT_TREFWOORDEN)
        return {"categorie": categorie, "hot": is_hot, "is_social": True}
    ...
    for feed_config in FEEDS:
        ...
        try:
            ...
            for entry in feed.entries[:30]:  # max 30 per feed
                ...
                classificatie = classificeer(titel, samenvatting, feed_config['naam'])
                ...
                alle_items.append({
                    ...
                    "is_social": classificatie.get('is_social', False),
                })
